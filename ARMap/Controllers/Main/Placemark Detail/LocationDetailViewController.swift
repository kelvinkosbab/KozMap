...
class LocationDetailViewController : BaseViewController, NSFetchedResultsControllerDelegate, DesiredContentHeightDelegate, DismissInteractable, KeyboardFrameRespondable, PlacemarkAPIDelegate {
  ...
  private static func newViewController() -> LocationDetailViewController {
    return self.newViewController(fromStoryboardWithName: "LocationDetail")
  }
  ...
  func reloadNavigationItems() {
    ...
    switch self.placemark?.placemarkType ?? .myPlacemark {
    case .myPlacemark:
    ...
    case .food:
      if placemark.isFavorite {
        self.navigationItem.rightBarButtonItem = UIBarButtonItem(image: #imageLiteral(resourceName: "assetStar"), style: .plain, target: self, action: #selector(self.favoriteButtonSelected))
      } else {
        self.navigationItem.rightBarButtonItem = UIBarButtonItem(image: #imageLiteral(resourceName: "assetStarEmpty"), style: .plain, target: self, action: #selector(self.favoriteButtonSelected))
      }
    case .mountain: break
    }
  }
  ...
  @objc func favoriteButtonSelected() {
    if let placemark = self.placemark {
      placemark.isFavorite = !placemark.isFavorite
      MyDataManager.shared.saveMainContext()
      self.reloadNavigationItems()
    }
  }
