...
@IBDesignable class GradientSlider: UIControl {
  ...
  required init?(coder aDecoder: NSCoder) {
    ...
    minColor = aDecoder.decodeObject(forKey: "minColor") as? UIColor ?? .lightGray
    maxColor = aDecoder.decodeObject(forKey: "maxColor") as? UIColor ?? .darkGray
  ...
  private func updateTrackColors() {
    ...
    let cnt = 40
    let step:Float = 1.0 / Float(cnt)
    let locations = (0...cnt).map { (i) -> NSNumber in
      return NSNumber(value: (step * Float(i)))
    }
    _trackLayer.colors = locations.map { (hue) -> CGColor in
      return UIColor(hue: CGFloat(hue.floatValue), saturation: s, brightness: l, alpha: a).cgColor
    }
  ...
  var currentColor: UIColor {
    ...
    guard self.hasRainbow else {
      // TODO: - Future determine color based on non-rainbow slider
      return .clear
    }
    ...
    return UIColor(hue: self._value, saturation: s, brightness: l, alpha: a)
  }
}
